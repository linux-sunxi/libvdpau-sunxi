...
static int sunxi_disp1_5_set_video_layer(struct sunxi_disp *sunxi_disp, int x, int y, int width, int height, output_surface_ctx_t *surface)
{
	...
	disp->video_info.fb.addr[0] = surface->yuv->data->phys;
	disp->video_info.fb.addr[1] = surface->yuv->data->phys + surface->vs->luma_size;
	disp->video_info.fb.addr[2] = surface->yuv->data->phys + surface->vs->luma_size + surface->vs->chroma_size / 2;
